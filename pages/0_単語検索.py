...
df_merge = pd.merge(df,df2).drop(columns=["tmc","tree_def"]).rename(columns={"tmc_def":"分野","def":"説明"})
...
df3 = agg_applicant(tmcs)

selected_df3 = df3[df3["f_term"].str.contains("|".join(options))]


with st.expander("分野関連ネットワーク図",):
    df_draw = selected_df3[selected_df3["appcount"]>3].rename(columns={"appl_name":"source","f_term":"target","appcount":"value"})
    df_draw["weight"] = df_draw["value"]
    df_draw["color"] = "blue"


#ネットワークづくり
    pyvis_G = re_construct_network(df_draw,att_applicant,df_merge[["fterm","分野","説明"]])
    pyvis_G.write_html("mygraph.html")

    with open('mygraph.html', 'rb') as f:
        st.download_button('ネットワーク図ダウンロード', f, file_name='mygraph.html')  # Defaults to 'application/octet-stream'
...
df_dic = {}
for i in tmcs:
    df_dic[i] = pd.merge(selected_df3[selected_df3["f_term"] == i],df_merge,left_on="f_term",right_on="fterm")
    #st.write(df_dic[i])
    ...
    for k,v in df_dic.items():
        try:
            ...
            if(df_dic[k]["appcount"].sum()<6):
                ...
                fig1 = px.bar(df_dic[k].sort_values(by="appcount"), y='appl_name', x='appcount',
                #title= k + "分野毎出願人上位",
                #facet_col="f_term",
                 text="appcount"
